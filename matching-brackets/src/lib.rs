...
pub fn brackets_are_balanced(string: &str) -> bool {
    ...
    let mut chars: Vec<&str> = string.split("").filter(|x| !x.trim().is_empty()).collect();
    println!("2 - Handled brackets: {:?}", chars);

    match chars.len() {
        x if x == 0 => return true,
        x if x % 2 != 0 => return false,
        _ => return check_pairs(&mut chars),
    }
}

fn check_pairs(brackets: &mut Vec<&str>) -> bool {
...
fn remove_matches(brackets: &mut Vec<&str>) -> &mut Vec<&str> {
